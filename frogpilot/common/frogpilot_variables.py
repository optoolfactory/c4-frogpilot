...
class FrogPilotVariables:
  ...
  def update(self, started):
    ...
    steerKp = CP.lateralTuning.pid.kp if CP.lateralTuning.which() == "pid" else CP.lateralTuning.torque.kp
    ...
    toggle.available_models = (self.params.get("AvailableModels") or "") + f",{default["DrivingModel"]}"
    toggle.available_model_names = (self.params.get("AvailableModelNames") or "") + f",{default["DrivingModelName"]}"
    downloaded_models = [model for model in toggle.available_models.split(",") if all((MODELS_PATH / f"{model}_{filename}").is_file() for filename, _ in TINYGRAD_FILES)]
    model_versions = (self.params.get("AvailableModelVersions") or "") + f",{default["DrivingModelVersion"]}"
    ...
    if toggle.model_randomizer:
      if not started:
        ...
        toggle.model = random.choice(selectable_models) if selectable_models else default["DrivingModel"]
      ...
      model = ((self.params.get("DrivingModel") if tuning_level >= level["DrivingModel"] else default["DrivingModel"]) or default["DrivingModel"]).removesuffix("_default")
      if model in downloaded_models:
        ...
        toggle.model_name = default["DrivingModelName"]
        toggle.model_version = default["DrivingModelVersion"]
    ...
    toggle.subaru_sng = toggle.car_make == "subaru" and not (CP.flags & SubaruFlags.GLOBAL_GEN2 or CP.flags & SubaruFlags.HYBRID) and (self.params.get_bool("SubaruSNG") if tuning_level >= level["SubaruSNG"] else default["SubaruSNG"])
