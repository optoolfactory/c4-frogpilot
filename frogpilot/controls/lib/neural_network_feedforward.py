...
class LatControlNNFF(LatControl):
  def __init__(self, CP, CI):
    ...
    self.use_steering_angle = self.torque_params.useSteeringAngleDEPRECATED
  ...
  def update(self, active, CS, VM, params, steer_limited_by_safety, desired_curvature, curvature_limited, model_data, frogpilot_toggles):
    ...
    if not active:
      ...
      if self.use_steering_angle:
        ...
        actual_curvature_llk = CS.yawRate / CS.vEgo
      ...
      if self.nnff_loaded and frogpilot_toggles.nnff or frogpilot_toggles.nnff_lite:
        ...
        if self.nnff_loaded and model_good and frogpilot_toggles.nnff:
          ...
          pitch = 0
          roll = params.roll
          if len(model_data.orientation.y) >= 1:
          ...
          future_lateral_accels = [np.interp(time, ModelConstants.T_IDXS[:CONTROL_N], model_data.acceleration.y) for time in adjusted_future_times]
