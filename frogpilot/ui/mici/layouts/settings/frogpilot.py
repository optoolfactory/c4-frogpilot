...
class PondPairButton(BigButton):
  ...
  def _handle_mouse_release(self, mouse_pos: MousePos):
    ...
    if self._paired:
      ...
      gui_app.set_modal_overlay(BigConfirmationDialogV2("slide to unpair", "icons_mici/settings/icon_frog.png", red=True, confirm_callback=unpair_confirmed))
    else:
      self.set_enabled(False)
...
class QRCodeDialog(NavWidget):
  def __init__(self):
    ...
    self._qr_texture = generate_qr_code()
  ...
  def _render(self, rect: rl.Rectangle):
    ...
    scale = min(rect.width, qr_area_height) / self._qr_texture.height

    qr_height = self._qr_texture.height * scale
    qr_width = self._qr_texture.width * scale

    x = rect.x + (rect.width - qr_width) / 2
    y = rect.y + (qr_area_height - qr_height) / 2

    rl.draw_texture_ex(self._qr_texture, rl.Vector2(x, y), 0.0, scale, rl.WHITE)

    self._label.set_position(rect.x, y + qr_height + 10)
