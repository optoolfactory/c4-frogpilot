...
class LongControl:
  ...
  def update(self, active, CS, a_target, should_stop, accel_limits, frogpilot_toggles):
    ...
    if self.long_control_state == LongCtrlState.off:
      ...
      if output_accel > self.frogpilot_toggles.stopAccel:
        output_accel = min(output_accel, 0.0)
        output_accel -= self.frogpilot_toggles.stoppingDecelRate * DT_CTRL
      self.reset()

    elif self.long_control_state == LongCtrlState.starting:
      output_accel = self.frogpilot_toggles.startAccel
      self.reset()
